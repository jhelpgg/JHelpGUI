      ...
      implements JHelpLayoutSmooth
{
   ...
   public Dimension computePreferredSize(final JHelpPanelSmooth container)
   {
      ...
      for(int i = 0; i < count; i++)
      {
         ...
         if(componentSmooth.isVisible() == true)
         {
            ...
            cellWidth = Math.max(cellWidth, (preferred.width + (constraintsSmooth.width >> 1)) / constraintsSmooth.width);
            cellHeight = Math.max(cellHeight, (preferred.height + (constraintsSmooth.height >> 1)) / constraintsSmooth.height);
         }
      }
   ...
   public Rectangle layoutComponents(final JHelpPanelSmooth container, final int x, final int y, final int parentWidth, final int parentHeight)
   {
      ...
      for(int i = 0; i < count; i++)
      {
         ...
         {
            constraintsSmooth = (JHelpTableConstraintsSmooth) pair.element2;
            componentSmooth.setBounds(x + ((constraintsSmooth.x - xMin) * cellWidth), y + ((constraintsSmooth.y - yMin) * cellHeight), constraintsSmooth.width * cellWidth, constraintsSmooth.height * cellHeight);
         }
      }

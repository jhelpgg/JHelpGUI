      ...
      extends SmoothShape
{
   ...
   protected ShapeInformation computeShape(final int x, final int y, final int width, final int height, final ShadowLevel level)
   {
      ...
      final Ellipse2D background = new Ellipse2D.Double(x, y, width - numberOfPixels, height - (numberOfPixels << 1));
      final Rectangle bounds = background.getBounds();
      final Rectangle inside = new Rectangle(bounds.x + (bounds.width >> 2), bounds.y + (bounds.height >> 2), bounds.width >> 1, bounds.height >> 1);
   ...
   {
      final int numberOfPixels = level.getNumberOfPixels() + 1;
      return new Insets(height >> 1, width >> 1, (height >> 1) + (numberOfPixels << 1), (width >> 1) + numberOfPixels);
   }
}
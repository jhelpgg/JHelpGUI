      ...
      implements MouseInputListener, Runnable
{
   ...
   /**
    * Sensibility of the drag start. That is to say the number of darg pixel before the drag is consider to start
    */
   private static final int             DETACH_SENSIBIITY = 10;
   ...
   public void run()
   {
      ...
      int wayX = UtilMath.sign(this.oldPosition.x - position.x);
      int wayY = UtilMath.sign(this.oldPosition.y - position.y);
      while((position.x != this.oldPosition.x) || (position.y != this.oldPosition.y))
      {
         ...
         position.y += wayY;
         this.dialog.setLocation(position);
         wayX = UtilMath.sign(this.oldPosition.x - position.x);
         wayY = UtilMath.sign(this.oldPosition.y - position.y);
      }

      ...
      implements ParseXMLlistener, Constants
{
   ...
   private void writeLine(final String... strings) throws ExceptionXML
   {
      try
      {
         ...
         {
            this.bufferedWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(this.currentFile)));
         }
      ...
      {
         throw new ExceptionXML(UtilText.concatenate("Issue while writing ", strings, " in ", this.currentFile.getAbsolutePath()), exception);
      }
   }
   ...
   public void endMarkup(final String markupName) throws ExceptionXML
   {
      ...
      Debug.printTodo("Implements markup " + markupName);
   }
   ...
   public void startMakup(final String markupName, final Hashtable<String, String> parameters) throws ExceptionXML
   {
      if(Constants.MARKUP_FRAME.equals(markupName) == true)
      {
         ...
         this.writeLine(" * {");
         this.writeLine(" *    ", name, " ", String.valueOf((char) ((name.charAt(0) - 'A') + 'a')), name.substring(1), "= new ", name, "();");
         this.writeLine(" *    ", String.valueOf((char) ((name.charAt(0) - 'A') + 'a')), name.substring(1), ".setVisible(true);");
         this.writeLine(" * }");

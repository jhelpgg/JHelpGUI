      ...
      extends JPanel
{
         ...
         implements MouseListener, MouseMotionListener
   {
      ...
      public void mouseClicked(final MouseEvent mouseEvent)
      {
         ...
         final int information = (Integer) labelJHelpImage.getAdditionalInformation();

         if(JHelpImageExplorer.this.currentFiles[information] != null)
         {
            JHelpImageExplorer.this.selected = information;
            labelJHelpImage.setSelected(true);
   ...
   public JHelpImageExplorer()
   {
      ...
      for(int i = 0; i < JHelpImageExplorer.NUMBER_MAXIMUM_OF_IMAGES; i++)
      {
         ...
         this.labelJHelpImages[i].setAdditionalInformation(i);
      ...
      this.imageFileFilter.setAcceptDirectory(false);
      this.selected = -1;
      this.firstIndex = 0;
   ...
   void actionMoveSelection()
   {
      ...
      if((directory == null) || (directory.getAbsoluteFile().equals(this.currentDirectory.getAbsolutePath()) == true))
      {
         return;
      }
   ...
   final void changeCurrentDirectory(final File directory)
   {
      ...
      this.actionChooseDirectory.setName(directory.getAbsolutePath());
      this.firstIndex = 0;
   ...
   final void removeSelection()
   {
      ...
      {
         JHelpImageExplorer.this.labelJHelpImages[JHelpImageExplorer.this.selected].setSelected(false);
      }
    ...
    * 
    * @param startSelected
    *           Indicates if its enough to start with the selected image (Image before didn't need to be updated)
    */
   final void updateFiles(final boolean startSelected)
   {
      ...
      if((startSelected == true) && (this.selected >= 0))
      {
         for(int i = this.selected + 1; i < number; i++)
         {
            image = this.labelJHelpImages[i].getJHelpImage();
            ...
            {
               this.labelJHelpImages[i - 1].setJHelpImage(image);
            }
      ...
      for(int i = start; i < number; i++)
      {
         ...
         try
         {
            ...
            if(image == null)
            {
               Debug.println(DebugLevel.WARNING, "Delete file : ", this.currentFiles[i].getAbsolutePath());
            ...
            {
               this.labelJHelpImages[i].setJHelpImage(image);
            }
         }
         catch(final Exception exception)
         {
            Debug.printException(exception, "Can't load : ", this.currentFiles[i].getAbsolutePath());

            this.labelJHelpImages[i].removeImageWithoutChangeSize();
         }
         catch(final Error error)
         {
            Debug.printError(error, "Can't load : ", this.currentFiles[i].getAbsolutePath());
      ...
      this.actionUp.setName(UtilText.concatenate("UP (", this.firstIndex / 100, "/", numberOfPage, ")"));
      this.actionDown.setName(UtilText.concatenate("DOWN (", Math.max(0, (this.files.size() - this.firstIndex - 100) / 100), "/", numberOfPage, ")"));
   ...
   void updateSelection()
   {
      ...
      try
      {
         ...
         this.preview.setJHelpImage(JHelpImage.loadImageThumb(file, 256, 256));

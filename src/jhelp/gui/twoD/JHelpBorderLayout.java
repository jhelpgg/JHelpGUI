      ...
      extends JHelpLayout
{
    ...
    * 
    * @param children
    *           Children to compute bounds
    * @param parentWidth
    *           Parent width (if -1, means not know now)
    * @param parentHeight
    *           Parent height (if -1, means not know now)
    * @return Component computed bounds
    * @see jhelp.gui.twoD.JHelpLayout#computeBounds(java.util.ArrayList, int, int)
    */
   @Override
   public Rectangle computeBounds(final ArrayList<Pair<JHelpComponent2D, JHelpConstraints>> children, int parentWidth, int parentHeight)
   {
      int top = 0;
      int left = 0;
      int right = 0;
      ...
      for(final Pair<JHelpComponent2D, JHelpConstraints> child : children)
      {
         ...
         {
            case BOTTOM:
               child.element1.setBounds(left, parentHeight - bottom, centerWidth, bottom);
            break;
            case BOTTOM_EXPANDED:
               child.element1.setBounds(0, parentHeight - bottom, parentWidth, bottom);
            break;
            case BOTTOM_LEFT:
               child.element1.setBounds(0, parentHeight - bottom, left, bottom);
            break;
            case BOTTOM_RIGHT:
               child.element1.setBounds(parentWidth - right, parentHeight - bottom, right, bottom);
            break;
            case LEFT:
               child.element1.setBounds(0, top, left, centerHeight);
            break;
            case LEFT_EXPANDED:
               child.element1.setBounds(0, 0, left, parentHeight);
            break;
            case RIGHT:
               child.element1.setBounds(parentWidth - right, top, right, centerHeight);
            break;
            case RIGHT_EXPANDED:
               child.element1.setBounds(parentWidth - right, 0, right, parentHeight);
            break;
            case TOP:
               child.element1.setBounds(left, 0, centerWidth, top);
            break;
            case TOP_EXPANDED:
               child.element1.setBounds(0, 0, parentWidth, top);
            break;
            case TOP_LEFT:
               child.element1.setBounds(0, 0, left, top);
            break;
            case TOP_RIGHT:
               child.element1.setBounds(parentWidth - right, 0, right, top);
            break;
            case CENTER:
               child.element1.setBounds(left, top, centerWidth, centerHeight);
            break;
         }
      }

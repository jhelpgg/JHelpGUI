      ...
      extends JPanel
{
    ...
    * @author JHelp
    */
   class TaskFireCancel
         extends ThreadedSimpleTask<ColorChooserListener>
   {
      /**
       ...
       */
      @Override
      protected void doSimpleAction(final ColorChooserListener parameter)
      {
         ...
         extends ThreadedSimpleTask<ColorChooserListener>
   {
      ...
      protected void doSimpleAction(final ColorChooserListener parameter)
      {
         parameter.colorChoosed(ColorChooser.this, ColorChooser.this.currentColor);
      }
   }
    ...
    *           Y position
    * @param panel
    *           Panel where put components
    */
   private void addSetting(final String key, final JSpinner spinner, final JSlider slider, final int y, final JPanel panel)
   {
    ...
    * 
    * @param changeMethod
    *           Method that change the color value
    */
   void updateColorChooser(final ChangeMethod changeMethod)
   {
      ...
      switch(changeMethod)
      {
         ...
         case SLIDER_CHANGE:
            ...
            this.currentColor = (this.sliderAlpha.getValue() << 24) | (this.sliderRed.getValue() << 16) | (this.sliderGreen.getValue() << 8)
      ...
      this.imageColors.fillRectangle(ColorChooser.COLOR_SIZE, 0, ColorChooser.COLOR_SIZE, ColorChooser.COLOR_SIZE, this.currentColor);
      int index = 0;
      ...
      for(int y = 0; y < 8; y++)
      {
         for(int x = 0; x < 8; x++)
         {
            this.imageColors.fillRectangle(x * ColorChooser.PALETTE_SIZE, ColorChooser.COLOR_SIZE + (y * ColorChooser.PALETTE_SIZE), ColorChooser.PALETTE_SIZE,
                  ColorChooser.PALETTE_SIZE, this.palette[index]);
            this.imageColors.drawRectangle(x * ColorChooser.PALETTE_SIZE, ColorChooser.COLOR_SIZE + (y * ColorChooser.PALETTE_SIZE), ColorChooser.PALETTE_SIZE,
   ...
   protected void fireCancel()
   {
      synchronized(this.colorChooserListeners)
      {
         ...
         {
            ThreadManager.THREAD_MANAGER.doThread(this.taskFireCancel, colorChooserListener);
         }
      }
   }
   ...
   protected void fireColorChoose()
   {
      synchronized(this.colorChooserListeners)
      {
         for(final ColorChooserListener colorChooserListener : this.colorChooserListeners)
         {
            ThreadManager.THREAD_MANAGER.doThread(this.taskFireColorChoose, colorChooserListener);
         }
      }
   }

...
public class SensitiveAnimationBuilder
{
    ...
    * @param y
    *           Y location
    * @param positionReferenceType
    *           Relative position when build
    * @param windowShape
    *           Shape to use for the "window"
    * @param background
    *           Background to draw inside the "window"
    * @param sensitiveAnimationLayout
    *           Layout to use when add element
    */
   public SensitiveAnimationBuilder(final int id, final int x, final int y, final PositionReferenceType positionReferenceType, final ButtonShape windowShape,
         final JHelpDrawable background, final SensitiveAnimationLayout sensitiveAnimationLayout)
   {
      if(positionReferenceType == null)
      {
         throw new NullPointerException("positionReferenceType musn't be null");
      }

      if(windowShape == null)
      {
         throw new NullPointerException("windowShape musn't be null");
      }
      ...
      {
         throw new NullPointerException("sensitiveAnimationLayout musn't be null");
      }
    ...
    *           Element to add
    * @param sensitiveAnimationConstraints
    *           Constraints to use
    */
   public void append(final SensitiveElement sensitiveElement, final SensitiveAnimationConstraints sensitiveAnimationConstraints)
   {
      ...
      this.elements.add(new Pair<SensitiveElement, SensitiveAnimationConstraints>(sensitiveElement, sensitiveAnimationConstraints));
   }
   ...
   {
      final Rectangle bounds = this.sensitiveAnimationLayout.layout(this.elements, 0, 0, width, height);
      final PositionReference positionReference = new PositionReference(this.x, this.y, width, height, this.positionReferenceType);
      final Point location = positionReference.computeLocation(bounds.width, bounds.height);
      final SensitiveAnimation sensitiveAnimation = new SensitiveAnimation(this.id, location.x, location.y, bounds.width, bounds.height);
      sensitiveAnimation.setBackground(0);
      ...
      {
         sensitiveAnimation.addSensitiveElement(element.element1);
      }

...
public class PositionReference
{
    ...
    *           Type of reference (The "relativity")
    */
   public PositionReference(final int x, final int y, final int width, final int height, final PositionReferenceType positionReferenceType)
   {
   ...
   public PositionReference(final Rectangle bounds, final PositionReferenceType positionReferenceType)
   {
      if(bounds == null)
      {
         throw new NullPointerException("bounds musn't be null");
      }
   ...
   public Point computeLocation(final int width, final int height)
   {
      ...
      {
         case CENTER:
            return new Point(this.bounds.x + ((this.bounds.width - width) >> 1), this.bounds.y + ((this.bounds.height - height) >> 1));
         case CENTER_BOTTOM:
            return new Point(this.bounds.x + ((this.bounds.width - width) >> 1), (this.bounds.y + this.bounds.height) - height);
         case CENTER_TOP:
         ...
         case RIGHT_BOTTOM:
            return new Point((this.bounds.x + this.bounds.width) - width, (this.bounds.y + this.bounds.height) - height);
         case RIGHT_CENTER:
            return new Point((this.bounds.x + this.bounds.width) - width, this.bounds.y + ((this.bounds.height - height) >> 1));
         case RIGHT_TOP:

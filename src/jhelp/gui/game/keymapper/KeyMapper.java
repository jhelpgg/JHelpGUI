      ...
      implements MouseSensitiveAreaListener, CaptureKeyCodeListener, EventManager
{
   ...
   static
   {
      ...
      image.fillRectangle(0, 0, KeyMapper.SIZE_MASK, KeyMapper.SIZE_MASK, new JHelpGradient(0xFFF0F000, 0xFF00F0F0, 0xFFF000F0, 0xFFFFFFFF));
    ...
    * @param colorBackground
    *           Color background, used if <b>imageBackground</b> is {@code null}
    * @param imageText
    *           Image foreground (can be {@code null})
    * @param colorText
    *           Color foreground, used if <b>imageText</b> is {@code null}
    * @param imageKeyForeground
    *           Image key foreground (can be {@code null})
    * @param keyForeground
    *           Color key foreground, used if <b>imageKeyForeground</b> is {@code null}
    * @param imageKeyBackground
    *           Image key background (can be {@code null})
    * @param keyBackground
    *           Color key background, used if <b>imageKeyBackground</b> is {@code null}
    * @param select
    *           Selected border color
    * @param normal
    *           Normal border color
         ...
         final int keyBackground, final int select, final int normal, final JHelpFont font)
   {
      ...
      final int totalWidth = KeyMapper.SPACE_TEXT_KEY + firstColumWidth + KeyMapper.SPACE_TEXT_KEY + KeyMapper.SIZE_MASK + KeyMapper.SPACE_COLUMN + //
      ...
      {
         this.imageKeyMapper.fillShape(new RoundRectangle2D.Double(0, 0, totalWidth, totalHeight, 20, 20), imageBackground);
      }
      else
      {
         this.imageKeyMapper.fillShape(new RoundRectangle2D.Double(0, 0, totalWidth, totalHeight, 20, 20), colorBackground);
      }

      // Column 1
      int x = KeyMapper.SPACE_TEXT_KEY;
      int y = KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, firstColumWidth, height, bounds.get(ActionKey.ACTION_MENU), ActionKey.ACTION_MENU, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, firstColumWidth, height, bounds.get(ActionKey.ACTION_ACTION), ActionKey.ACTION_ACTION, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, firstColumWidth, height, bounds.get(ActionKey.ACTION_CANCEL), ActionKey.ACTION_CANCEL, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, firstColumWidth, height, bounds.get(ActionKey.ACTION_EXIT), ActionKey.ACTION_EXIT, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, firstColumWidth, height, bounds.get(ActionKey.ACTION_FIRE), ActionKey.ACTION_FIRE, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      ...
      this.drawButton(x, y, secondColumWidth, height, bounds.get(ActionKey.ACTION_LEFT), ActionKey.ACTION_LEFT, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;

      // Column 3
      x += secondColumWidth + KeyMapper.SPACE_TEXT_KEY + KeyMapper.SIZE_MASK + KeyMapper.SPACE_COLUMN;
      y = KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, thirdColumnWith, height, bounds.get(ActionKey.ACTION_UP), ActionKey.ACTION_UP, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, thirdColumnWith, height, bounds.get(ActionKey.ACTION_DOWN), ActionKey.ACTION_DOWN, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, thirdColumnWith, height, bounds.get(ActionKey.ACTION_FIRE_2), ActionKey.ACTION_FIRE_2, imageText, colorText, font);

      // Column 4
      x += thirdColumnWith + KeyMapper.SPACE_TEXT_KEY + KeyMapper.SIZE_MASK + KeyMapper.SPACE_COLUMN;
      y = KeyMapper.SPACE_COLUMN + height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, fourthColumWidth, height, bounds.get(ActionKey.ACTION_RIGHT), ActionKey.ACTION_RIGHT, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;

      // Column 5
      x += fourthColumWidth + KeyMapper.SPACE_TEXT_KEY + KeyMapper.SIZE_MASK + KeyMapper.SPACE_COLUMN;
      y = KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, fithcolumnWidth, height, bounds.get(ActionKey.ACTION_PREVIOUS), ActionKey.ACTION_PREVIOUS, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, fithcolumnWidth, height, bounds.get(ActionKey.ACTION_NEXT), ActionKey.ACTION_NEXT, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, fithcolumnWidth, height, bounds.get(ActionKey.ACTION_START), ActionKey.ACTION_START, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, fithcolumnWidth, height, bounds.get(ActionKey.ACTION_PAUSE), ActionKey.ACTION_PAUSE, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      this.drawButton(x, y, fithcolumnWidth, height, bounds.get(ActionKey.ACTION_SPECIAL), ActionKey.ACTION_SPECIAL, imageText, colorText, font);
      y += height + KeyMapper.SPACE_COLUMN;
      ...
      for(final ActionKey actionKey : ActionKey.values())
      {
         ...
         if(mask != null)
         {
            if(this.imageKeyForeground == null)
            {
               ...
               {
                  image.paintMask(bounds.x, bounds.y, mask, this.keyForeground, this.imageKeyBackground, 0, 0, true);
               }
            }
            else
            {
               if(this.imageKeyBackground == null)
               {
                  image.paintMask(bounds.x, bounds.y, mask, this.imageKeyForeground, 0, 0, this.keyBackground, true);
               }
               else
               {
                  image.paintMask(bounds.x, bounds.y, mask, this.imageKeyForeground, 0, 0, this.imageKeyBackground, 0, 0, true);
               }
            }
         }
   ...
   public void capturedkeyCode(final int keycode)
   {
      ...
      this.selected = this.game2d.setKeyCode(this.selected, keycode);
   ...
   public void removeFromJHelpGame2D()
   {
      ...
      this.game2d.setEventManager(this.eventManager);
   ...
   public void showInJHelpGame2D(final JHelpGame2D game2d)
   {
      if(game2d == null)
      {
         throw new NullPointerException("game2d musn't be null");
      }
      ...
      this.game2d = game2d;
      this.eventManager = game2d.getEventManager();
      game2d.setEventManager(this);

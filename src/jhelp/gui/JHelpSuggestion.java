...
import jhelp.gui.model.ListFiterableModel;
 ...
 * @author JHelp <br>
 * @param <INFORMATION>
 *           Information type carry by suggestions
 */
public class JHelpSuggestion<INFORMATION>
{
   ...
   private final ListFiterableModel<SuggestionElement<INFORMATION>> suggestionModel;
    ...
    *
    * @param current
    *           Indicates if have to delete current character or previous one
    */
   ...
   private void selectChoiceIndex(final int index)
   {
      ...
      this.suggestionList.addListSelectionListener(this.eventManager);
      final String details = this.suggestionList.getSelectedValue().getHelpDetails();
    ...
    *           Key code
    * @param modifiers
    *           Key modifiers (control, shift, alt, altGr, meta)
    */
   ...
   void keyTyped(final int keyCode, final char keyChar)
   {
      if(keyChar >= 32)
      {
         try
         {
            this.attachedComponent.getDocument().insertString(this.attachedComponent.getCaretPosition(), String.valueOf(keyChar), null);
         }
         catch(final BadLocationException exception)
         {
            Debug.printException(exception);
   ...
   void positionCaretChange()
   {
      ...
      if(index < caretPosition)
      {
         ...
         final String regex = UtilText.removeAccent(text.substring(index + 1, caretPosition + 1).replaceAll("[^a-zA-Z0-9]", "").toLowerCase()).trim();
         ...
         if(size > 0)
         {
            ...
            if(document instanceof StyledDocument)
            {
               ...
               final AttributeSet attributeSet = ((StyledDocument) document).getCharacterElement(caretPosition).getAttributes();
               font = new JHelpFont(StyleConstants.getFontFamily(attributeSet), StyleConstants.getFontSize(attributeSet), StyleConstants.isBold(attributeSet),
                     StyleConstants.isItalic(attributeSet), StyleConstants.isUnderline(attributeSet));
            }
            else
            {
   ...
   void select()
   {
      ...
      final String word = this.suggestionList.getSelectedValue().getKeyWord();
    ...
    *            If list is <code>null</code> or one of keywords is <code>null</code>
    */
    ...
    *           Keyword
    * @param information
    *           Information associated (Use <code>null</code> if none)
    * @throws NullPointerException
    *            If keyword is <code>null</code>
    */
    ...
    *           Keyword
    * @param information
    *           Information (Use <code>null</code> if none)
    * @param helpDetails
    *           Details (Use <code>null</code> if none)
    * @throws NullPointerException
    *            If keyword is <code>null</code>
    */
    ...
    *           Suggestions to add
    * @throws NullPointerException
    *            If list is <code>null</code> or one of suggestion is <code>null</code>
    */
    ...
    *
    * @param renderer
    *           New renderer (Can use <code>null</code> to return to default one)
    */

      ...
      implements EventManager
{
   ...
   protected void gameRefresh(final JHelpImage image)
   {
         ...
         if(tile != SnakeFrame.EMPTY)
         {
            ...
            switch(tile)
            {
               case WALL:
                  ...
                  image.fillRectangle((x * w) + (w / 10), (y * h) + (h / 10), w - (w / 5), h - (h / 5), SnakeFrame.COLOR_WALL_INSIDE);
               break;
            }
         }
      }
      ...
      xx = UtilMath.modulo(position.x + this.wayX, SnakeFrame.BOARD_WIDTH);
      yy = UtilMath.modulo(position.y + this.wayY, SnakeFrame.BOARD_HEIGHT);
